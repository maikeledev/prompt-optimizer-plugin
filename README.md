...
5.  Para activar la **auto-optimización**, marca la casilla `Auto Optimize`. despues de agregar `?UPE` al final del texto, la extensión la mejorará automáticamente tras una breve pausa(2s).
...
Si activaste `Auto Optimize` en la configuración, simplemente escribe un prompt en una línea y termínalo con un signo de interrogación y la palabra UPE (`?UPE`). Después de dos segundos, la línea se optimizará sola.
